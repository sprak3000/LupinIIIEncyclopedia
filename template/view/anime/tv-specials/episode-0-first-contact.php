<div class="container">

  <div class="row">

    <div class="col-md-12">

      <div class="tab-v1">

        <ul class="nav nav-tabs">
          <li class="active"><a data-toggle="tab" href="#overview">Overview</a></li>
          <li><a data-toggle="tab" href="#cast">Cast</a></li>
          <li><a data-toggle="tab" href="#staff">Staff</a></li>
          <li><a data-toggle="tab" href="#media">Media</a></li>
          <li><a data-toggle="tab" href="#soundtracks">Soundtracks</a></li>
        </ul>

        <div class="tab-content">

          <div id="overview" class="tab-pane fade in active">
            <div class="row">
              <div class="col-md-10">
                <dl class="dl-horizontal">
                  <dt>Japanese Title:</dt><dd><em>Rupan Sansei: Episode:0 faasuto kontakuto</em></dd>
                  <dt>Jacket Color:</dt><dd>Red</dd>
                  <dt>Release Date:</dt><dd>2002 July 26</dd>
                  <dt>Run Time:</dt><dd>90 minutes</dd>
                </dl>
                <div class="headline">
                  <h4>Synopsis</h4>
                </div>
                <p>
                  Jigen sits in a bar; next to him is a Elana, a reporter. She starts her tape recorder and asks Jigen
                  about the first time he met Lupin III. They walk out of the bar and into an alley. Jigen strikes a
                  match and remembers...
                </p>
                <p>
                  Jigen's tale revolves around a millionaire named Galbez and an ancient Japanese cylinder made from
                  some green metal. The cylinder contains a scroll that tells how to forge an indestructible metal;
                  Lupin wanted to steal it, and it was Jigen's job to stop him along with Galbez's bodyguard Shade.
                </p>
                <p>
                  The 30th anniversary of the Lupin anime brings our favorite friends together for the first time as
                  Lupin races to find way to open the cylinder before Jigen, Shade, and Galbez track him down.
                </p>
              </div>
              <div class="col-md-2">
                <img class="img-responsive" src="/dist/asset/img/anime/tv-specials/episode-0-first-contact/episode-0-first-contact.jpg" alt="Episode:0 First Contact DVD Cover" />
              </div>
            </div>
          </div>

          <div id="cast" class="tab-pane fade in">
              <?php echo $app->render('partial/cast.php', ['partial' => true, 'annData' => $annData]); ?>
          </div>

          <div id="staff" class="tab-pane fade in">
              <?php echo $app->render('partial/staff.php', ['partial' => true, 'annData' => $annData]); ?>
          </div>

          <div id="soundtracks" class="tab-pane fade in">
            <div class="row margin-bottom-30">

              <div class="col-md-8">
                <h2>Lupin the Third Episode:0 First Contact TV Special Original Soundtrack</h2>
                <ol>
                  <li>Lupin the Third (A tarde cai) &lt;TV Size&gt;</li>
                  <li>First Contact</li>
                  <li>Theme from Lupin III '89 &lt;TV Size&gt;</li>
                  <li>Cram of Hermes</li>
                  <li>Magnum Dance &lt;TV Size&gt;</li>
                  <li>Brad's Death ~ Zenigata's Arrival</li>
                  <li>Stolen Moments (Flor espacial) &lt;TV Size&gt;</li>
                  <li>Saudades Do Rio &lt;TV Size&gt;</li>
                  <li>Love Squall (Po do amor)</li>
                  <li>Magnum Dance &lt;TV Size&gt;</li>
                  <li>Enemies or Friends, a mixture of 5 people</li>
                  <li>Isn't It Lupintic &lt;TV Size&gt;</li>
                  <li>Climax of the Tresure Battle</li>
                  <li>Lupin the Third Theme '78 &lt;2002 Version TV Size&gt;</li>
                  <li>That's a Duel between Decker and Zenigata Duel</li>
                  <li>Going? Out On The Town &lt;TV Size&gt;</li>
                  <li>Awe of Zantetsuken</li>
                  <li>Memory of Smile (Pequena luz) &lt;TV Size&gt;</li>
                </ol>
              </div>

              <div class="col-md-4">
                <div class="rounded-2x service-block-light">
                  <img class="img-responsive center-block" src="/dist/asset/img/anime/tv-specials/episode-0-first-contact/soundtrack/lupin-the-third-episode-0-first-contact-tv-special-original-soundtrack.jpg" alt="Lupin the Third Episode:0 First Contact TV Special Original Soundtrack CD cover" />
                  <dl class="dl-horizontal">
                    <dt>Company:</dt><dd>Vap</dd>
                    <dt>Catalog #:</dt><dd>VPCG-84770</dd>
                    <dt>Release Date:</dt><dd>2002 October 23</dd>
                    <dt>Price:</dt><dd>2300&yen;</dd>
                    <dt># of Discs:</dt><dd>1</dd>
                    <dt>Artist(s):</dt><dd>Ohno Yuji (all tracks)</dd>
                  </dl>
                </div>
              </div>

            </div>

          </div>

          <div id="media" class="tab-pane fade in">
              <?php echo $app->render('partial/media.php', ['partial' => true, 'images' => $images]); ?>
          </div>

        </div>

      </div>

    </div>

  </div>

</div>